...
/**
 * This class implements the methods that perform the various insertions and the
 * various queries, to manage the requests sent by the local nodes.
 */
public class RequestManager {
	...
	public RequestManager() {
		...
		this.mongoClient = new MongoClient(
				new MongoClientURI("mongodb+srv://" + username + ":" + password + "@cluster0" + db_host));
		this.mongoDatabase = mongoClient.getDatabase(databaseName);
		this.collection = mongoDatabase.getCollection(collectionName);
	}
	...
	public void addRequest(Request request) {
		...
		Bson condition = and(eq("id_requester", request.getIdRequester()),
				eq("serial", request.getRequestedBagSerial()), ne("state", RequestState.accepted.toString()));
		Bson condition2 = and(eq("serial", request.getRequestedBagSerial()),
		...
		if (!iterator.hasNext()) {
			...
			throw new RequestCloneNotSupportedException(
					"La richiesta che si vuole inserire è stata già accettata, rifiutata o inviata...");
		}
	...
	public void deleteRequest(Request request) {
		...
		Bson filter = and(eq("id_requester", request.getIdRequester()), eq("serial", request.getRequestedBagSerial()),
	...
	public void acceptRequest(Request request) {
		...
		if (collection.replaceOne(filter, editedRequest).getMatchedCount() != 0) {
			...
			filter = and(ne("id_requester", request.getIdRequester()), eq("serial", request.getRequestedBagSerial()),
					eq("state", RequestState.pending.toString()));
	...
	public void closeRequest(String serial) {
		...
		Bson filter = and(eq("id_requester", id_requester), eq("serial", serial),
	...
	public List<Document> getOurRequestsByState(RequestState state) {
		...
		Bson filter = and(eq("id_requester", id_requester), eq("state", state.toString()));
	 ...
	 * 
	 * boolean exists = false; for (String name :
	 * mongoDatabase.listCollectionNames()) {
	 * 
	 * if(name.equals("requestes")) exists=true; }
	 * 
	 * if(exists) return mongoDatabase.getCollection("requestes");
	 * mongoDatabase.createCollection("requestes"); return
	 * mongoDatabase.getCollection("requestes"); }
	 */
}