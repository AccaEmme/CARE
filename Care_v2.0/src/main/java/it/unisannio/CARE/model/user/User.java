...
public class User {
	...
	/*
	 * Il costruttore genera una sessione utente. Il costruttore genera l'MD5 della
	 * password data in input, per confrontarla. Quando l'amministratore crea un
	 * utente attribuendogli un ruolo, la password viene generata in automatico ed è
	 * visibile. Al primo cambio password dell'utente, viene eliminata la password
	 * temporanea, impostata la password cifrata e viene segnato l'ultimo cambio
	 * password.
	 */
	 ...
	 */
	/*
	 * public User(String username, String plainTextPassword) throws UserException,
	 * NullPasswordException { this.validateCredentials(username,plainTextPassword);
	 * this.username = username; this.setPassword(plainTextPassword); }
	 */
	...
	public User(String username, Password hiddenPassword) throws UserException, NullPasswordException {
		// this.validateCredentials(username,hiddenPassword.getHiddenPassword()); //il
		// metodo per prendere la password è getHiddenPassword
	 ...
	 */
	/*
	 * @SuppressWarnings("deprecation") public User(String username, Role role) { //
	 * This constructor can be invoked by Administrator creates users.
	 * 
	 * this.username = username; this.temppass =
	 * Password.generatePassword(Constants.USER_TEMPPASS_LENGTH);
	 * this.setPassword(temppass); this.role = role;
	 * 
	 ...
	 */
	/*
	 * public void validateCredentials(String username, String password) throws
	 * UserException, NullPasswordException { if (username == null) throw new
	 * UserException("The username cannot be null!"); if (password == null ||
	 * password == " ") throw new
	 * NullPasswordException("The password cannot be null!"); if (username.length()
	 * < 5 || password.length() < 5) throw new
	 * UserException("The username or the password cannot be shorter than 5 chars");
	 * }
	 */
	...
	public void setUsername(String username) {
		if (username.length() < 3)
			throw new IllegalArgumentException("Username size too short, should be greater than 3 chars.");
	...
	public void setEmail(String email) throws lllegalEmailException {
		if (!email.matches(Constants.RegexEmail))
			throw new lllegalEmailException("Email not valid");
		this.email = email;
	}
	 ...
	 */
	/*
	 * POSSIBILE IMPLEMENTAZIONE FUTURA. public static User getUser(UserDAO dao) {
	 * User user2Dao = new User( dao.getUsername(), // HTTP username
	 * dao.getPassword(), // HTTP plainTextPassword Role.valueOf(dao.getUserRole())
	 * // HTTP Role ); user2Dao.setEmail(dao.getEmail());
	 * 
	 * return user2Dao; }
	 */
