...
/**
 * Enumerator for the definition of the various types of blood groups and also
 * of those compatible for donations
 */
public enum BloodGroup {
	...
	private static HashMap<BloodGroup, List<BloodGroup>> canDonateTo = new HashMap() {
		{
			...
			put(ZEROneg,
					new ArrayList<BloodGroup>(Arrays.asList(Apos, Aneg, Bpos, Bneg, ZEROpos, ZEROneg, ABpos, ABneg)));
	...
	/**
	 **************************************************************************
	 * Method to preserve data and to deny the removal of elements from the
	 * recipient
	 * 
	 * @param b A BloodGroup object is passed
	 **************************************************************************
	...
	/**
	 **************************************************************************
	 * Method that returns the string of all BloodGroup enumerator values separated
	 * by the delimiter specified as an argument
	 * 
	...
	public static String delimitedValues(String delimiter) {
		/*
		 * suggested delimitator: pipeline "|" comma "," semicolon ";" tab "\t" dash "-"
		 * space " "
		 */
		return Arrays.stream(BloodGroup.values()).map(Enum::toString).collect(Collectors.joining(delimiter));
	}
}