...
public class Password {
	/*
	 * [RATIONAL] Hermann: Per evitare di dare troppe responsabilità alla classe
	 * "User", deleghiamo alla classe Password di cui l'Utente si compone, creando
	 * così una relazione, per avere la complessità sotto controllo. Non perché sia
	 * una parte logica, ma perché delegando le responsabilità aiuta una
	 * realizzazione di tipo qualitativo.
	 */

	String hiddenPassword;

	/*
	 * public Password() { String temppass = this.generatePassword(15);
	...
	/*
	 * public void setPlainTextPassword(String plainTextPassword) { if(
	 * validatePassword(plainTextPassword) ) { this.hiddenPassword = getMd5(
	...
	/**
	 ************************************************** ************************ Method for returning the hashcode with MD5 algorithm
	 * of the input value including the SALT
	 * 
	...
	public static String getBCrypt(String input) {
		...
		/*
		 * try { // Static getInstance method is called with hashing MD5 MessageDigest
		 * md = MessageDigest.getInstance("MD5");
		 * 
		 * // digest() method is called to calculate message digest // of an input
		 * digest() return array of byte byte[] messageDigest =
		 ...
		 * while (hashtext.length() < 32) { hashtext = "0" + hashtext; } return
	...
	public static String generatePassword(int length) {
		...
		// if( validatePassword(password.toString()) ) return password.toString(); else
		// generatePassword(length); // *** rischiamo loop?
		// return password;
	...
	/*
	 * Regex explaination of password secure requirements: Password must contain at
	 * least one digit [0-9]. Password must contain at least one lowercase Latin
	 * character [a-z]. Password must contain at least one uppercase Latin character
	 * [A-Z]. Password must contain at least one special character like ! @ # & ( ).
	...
	/**
	 ************************************************** ************************ Method that checks if the Pattern of the passw is
	 * correct
	 * 
	...
	public static boolean validatePlaintextPasswordPattern(final String givenPassword) {
		...
		if (givenPassword != null)
			...
			throw new IllegalPatternException(
					"Password.java validatePlaintextPasswordPattern: givenPassword is null. Value: " + givenPassword);

		// return matcher.matches();

		/*
		 * try { if( matcher.matches() ) return true; else throw new
		 * IllegalArgumentException("Password pattern conformity not valid");
		...
		// if( !matcher.matches() ) throw new IllegalArgumentException("Password pattern
		// conformity not valid");
	}
