...
/**
 * The BloodBag class contains all the information about the registered blood
 * bag, such as the serial number, creation date and expiration date and the
 * donor code.
 */
...
public class BloodBag implements Cloneable, Comparable<BloodBag> {
	...
	private String donatorCF; // =null; TODO: *** Attenzione al rischio di null pointer exception se
								// richiamato il donatorCF
	 ...
	 * 
	 * @param bloodGroup Object that is passed as a parameter to indicate which
	 *                   blood group it belongs to
	...
	public BloodBag(BloodGroup bloodGroup, String donatorCF) throws ParseException {
		/*
		 * Attenzione: new BloodBag() va usato solo per creare sacche perché incrementa
		 * il seriale.
		 */
	 ...
	 * @param valueOf   Enumerator: specifies the blood group among the defined
	 *                  groups
	 ...
	 **************************************************************************
	 */
	public BloodBag(Serial serial, BloodGroup valueOf, Date cd, Date ed, String donatorCF, BloodBagState valueOf2,
			String note2) {
	...
	private Date generateCreationDate() throws ParseException {
		// We can't allow user set a wrong "creation date", so the system will stamp the
		// right updated date.
		String datestr = serial.toString().substring(serial.toString().length() - 13, // extract date from serial
	...
	public void setCreationDate(Date creationDate) throws ParseException {
		...
		/*
		 * Si definisce il tempo limite di setCreationDate: non oltre -7gg prima, non
		 * futura a oggi.
		 */
		if (creationDate.before(dateLimitBeforeCreationDate))
			throw new IllegalDateException(Constants.ExceptionIllegalArgument_BloodBagNotValid + "creationDate "
					+ creationDate + " can't be < -7days " + dateLimitBeforeCreationDate, "/bloodbag/setdate");

		if (creationDate.after(dateLimitAfterCreationDate))
			throw new IllegalDateException(Constants.ExceptionIllegalArgument_BloodBagNotValid + "creationDate "
					+ creationDate + " can't be > today " + dateLimitAfterCreationDate, "/bloodbag/setdate");
	...
	private Date generateExpirationDate() throws ParseException {
		// We can't allow user set a wrong "expiration date", so the system will stamp
		// the right updated date.
		Calendar cal = Calendar.getInstance();
		cal.setTime(this.creationDate);
	...
	public void setExpirationDate(Date expirationDate) {
		if (!expirationDate.after(creationDate))
			throw new IllegalArgumentException(Constants.ExceptionIllegalArgument_BloodBagNotValid + "expirationDate "
					+ expirationDate + " not > creationDate " + creationDate);
	...
	/*
	 * private void setDonatorCF(String fisCode) { if(
	 * !fisCode.toUpperCase().matches(Constants.RegexDonatorCF) ) throw new
	 * IllegalFiscalCodeException(
	 * Constants.ExceptionIllegalArgument_BloodBagNotValid+"donatorCF "
	 * +donatorCF+" do not match pattern "+Constants.RegexDonatorCF, "/bloodbag/add"
	 ...
	 * 
	 * @return returns the bloodGroup object which refers to the type of blood bag
	 *         selected
	 **************************************************************************
	 */
	...
	public String toString() {
		return "{\"serial\": \"" + this.serial + "\"" + ", \"creation_date\": \"" + this.creationDate.getTime() + "\""
				+ ", \"donator\": \"" + this.donatorCF + "\"" + ", \"expiration_date\": \""
				+ this.expirationDate.getTime() + "\"" + ", \"group\": \"" + this.bloodGroup + "\"" + ", \"notes\": \""
				+ this.note + "\"" + ", \"state\": \"" + this.bloodBagState + "\"" + "}";
	}
	...
	public boolean equals(Object obj) {
		if (obj instanceof BloodBag) {
			...
			return (serial.equals(bBag.getSerial()) && bloodGroup.equals(bBag.getBloodGroup())
	...
	public void print() {
		System.out.println("Seriale: " + serial.toString() + ";\nGruppo: " + bloodGroup + ";\nData Creazione: "
				+ new SimpleDateFormat(Constants.DATE_FORMAT_STRING).format(creationDate) + ";\nData scadenza: "
				+ new SimpleDateFormat(Constants.DATE_FORMAT_STRING).format(expirationDate)
