...
public class LabelGenerator {
	...
	private void mergeImages() throws IOException {
		...
		File file0 = new File("Labels/scrap/" + this.serial + "-" + this.timestamp + ".png"); // immagine di sfondo:
																								// html
		BufferedImage im = ImageIO.read(file0);
		System.out.println("entro qui");
		// URL url2 = new
		// URL("QR_codes/IT-AV201000-ANEG-20210714-0000_1626254144766.png");
		// File file = new
		// File("QR_codes/IT-AV201000-ANEG-20210713-0000_1626274470375.png"); //solo di
		// test
		...
		/*
		 * al centro g.drawImage(im2, (im.getWidth() - im2.getWidth()) / 2,
		 * (im.getHeight() - im2.getHeight()) / 2, null);
		 */
		g.drawImage(im2, 25, 50, null);

		g.dispose();

		/*
		 * JFrame f = new JFrame(); f.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		 * f.getContentPane().add(new JLabel(new ImageIcon(im))); f.pack();
		 * f.setVisible(true);
		 */
	...
	private void renderHTML() throws IOException {
		...
		String htmlBody = "<p>&nbsp;</p>\n"
				...
				+ "<p style=\"text-align: center;\">.</p>\n" + "<p style=\"text-align: center;\">.</p>\n"
				+ "<p style=\"text-align: center;\">.</p>\n" + "<p style=\"text-align: center;\">&nbsp;</p>\n"
				+ "<p style=\"text-align: center;\">&nbsp;</p>\n" + "<p style=\"text-align: center;\">" + this.serial
