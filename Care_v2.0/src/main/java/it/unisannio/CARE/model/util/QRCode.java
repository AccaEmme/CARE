...
public class QRCode {
	...
	public QRCode(JSONObject objectToWrite) {
		...
		this.identifier = objectToWrite.get("serial").toString().replaceAll("\\s+", "_");
	...
	public QRCode(String objectToWrite, String identifier) {
		...
		this.identifier = identifier.replaceAll("\\s+", "_");
	}

	// tested
	public QRCode(BloodBag objectToWrite) {
		this.object = this.createBloodBagObject(objectToWrite).toJSONString();
		this.identifier = objectToWrite.getSerial().toString().replaceAll("\\s+", "_");
	}

	// tested
	public QRCode(BloodBagDAO objectToWrite) {
		this.object = this.createBloodBagObject(objectToWrite).toJSONString();
		this.identifier = objectToWrite.getSerial().replaceAll("\\s+", "_");
	}
	...
	public void createQRCode() {
		try {
			...
			if (this.customTimestamp != 0)
				...
				filePath = this.save_path + identifier + "_" + new Date().getTime() + ".png";
