...
public class P2PRequestController {
	...
	public JSONArray addp2pRequestRemote(@RequestBody RequestDAO request, @PathVariable String nodeid,
			@PathVariable String token) throws NodeNotFoundException, IOException, ParseException {
	...
	public JSONArray acceptRequest(@PathVariable String serial, @PathVariable String token) throws Exception {
		...
		if (requestDAO == null)
			...
			if (requestDAO.getState().equals(RequestState.accepted))
				...
				throw new RequestException("unexpected exception");
		}
