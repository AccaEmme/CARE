...
public class BloodBag implements BloodBagInterface, Cloneable, Comparable<BloodBag>{
	...
	public BloodBag( Serial s,BloodGroup b, Date creationD,Date expirationD,String donator,Node n,BloodBagState BagState,String not) throws ParseException {
		this.serial 		=s;
		this.setBloodGroup(b);
		this.setCreationDate(creationD);
		this.setExpirationDate(expirationD);
		this.setDonatorCF(donator);
		this.setNode(n);
		this.setNote(not);		/* TODO: gestirlo con classe Optional per evitare NullPointerException */
		this.setBloodBagState(BagState );
	}
