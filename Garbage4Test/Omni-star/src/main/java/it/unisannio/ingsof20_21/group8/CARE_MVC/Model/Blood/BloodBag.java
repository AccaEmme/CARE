...
public class BloodBag implements BloodBagInterface, Cloneable, Comparable<BloodBag>{
    ...
    public Document getDocument(){
    	Document document = new Document("serial",this.serial.toString())
			.append("blood-group",this.bloodGroup.toString())
			.append("location",this.creationDate)
            .append("role",this.expirationDate);
        if (this.donatorCF!=null)
            document.append("password_last_update",this.donatorCF);
        if (this.node!= null)
        	document.append("node", this.node)
        	.append("state", bloodBagState);
        if (this.note!=null)
        	document.append("note", this.note);
